      ...
      {faqData.map((section, i) => (
        <Box key={i} sx={{ mb: 6 }}>
          ...
          {section.items.map((faq, idx) => (
            ...
            >
              <AccordionSummary
                expandIcon={<ExpandMoreIcon sx={{ color: '#f27b96' }} />}
              >
                <Typography sx={{ fontWeight: 700, color:'#f27b96' }}>{faq.question}</Typography>
              </AccordionSummary>
              <AccordionDetails>
                {faq.answer.split('\n').map((line, index) => (
                  <Typography key={index} paragraph sx={{ color: '#6D6E71', fontSize: '0.875rem' }}>
                    {line}
                  </Typography>
                ))}
              </AccordionDetails>
